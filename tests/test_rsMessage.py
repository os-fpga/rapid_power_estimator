...
def test_get_message(message_code, message_type, expected_text):
    # Mock the get_message method to return an RsMessage with the provided parameters
    with patch.object(RsMessageManager, 'get_message', return_value=RsMessage(message_code, message_type, expected_text)):
...
def test_message_text_substitution():
    ...
    with patch.object(RsMessageManager, 'get_message', return_value=RsMessage(106, RsMessageType.INFO, "Hello, {name}!")):
